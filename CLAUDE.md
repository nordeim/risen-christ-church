...
- `test` in `vite.config.ts` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` — keeps `e2e/**` out of unit runs. **Note:** `src/test/setup.ts` exists (jest-dom + IntersectionObserver mock + scrollTo/scrollIntoView stubs + matchMedia stub, with Risen Christ fixtures) — `pnpm test` runs the 32 files / 175 unit suite.
...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ✅ | **32 files / 175 tests** (`ci-workflow` 4 + `repo-hygiene` 2 + `docs-contract` 8 + `cn` 5 + `nav` 7 + `content` 10 + `site` 8 + `massDay` 5 + `monogram` 7 + `Button` 11 + `SkipLink` 3 + `Accordion` 6 + `SafeImage` 6 + `Header` 17 + `BackToTop` 7 + `Reveal` 2 + `Ministries` 3 + `cta-bands` 6 + `worship-mass` 5 + `about-visuals` 4 + `event-chips` 3 + `give-featured` 2 + `card-affordances` 6 + `Timeline` 3 + `NotFound` 2 + `History` 2 + `Layout` 2 + `useScrollProgress` 4 + `useScrollSpy` 4 + `ScrollProgress` 2 + `head` 13 + `security-headers` 6) via `src/test/setup.ts` |
...
| `pnpm test:e2e` / `npm run test:e2e` | Playwright `chromium` — `playwright test` (7 specs — 48 tests: smoke 11 + navigation 8 + ministries 4 + give-faq 4 + enhancements 7 + enhancements-round5 6 + enhancements-round7 8) | ✅ | Risen Christ copy (91 Toa Payoh Central, He is risen, Velankanni, Toa Payoh NS19) |
...
> Before documenting a command as available, verify it in `package.json` scripts. The five-command gate is fully active (unit 32 files / 175 tests + 48 E2E green since the 2026-08-31 port).
...
Current status: **wired — 32 files / 175 tests + 48 E2E green (2026-08-31).** `vitest 3.2.6` (jsdom) + `@testing-library/react 16.2.0` + `jsdom 26.1.0` + `src/test/setup.ts` (jest-dom + IntersectionObserver + scrollTo/scrollIntoView + matchMedia) + `playwright 1.55.1` (chromium, 7 specs: smoke 11 + navigation 8 + ministries 4 + give-faq 4 + enhancements 7 + enhancements-round5 6 + enhancements-round7 8). Run `pnpm test` (unit), `pnpm test:watch` (watch), `pnpm test:coverage` (coverage), `pnpm test:e2e` (E2E, `webServer` → `pnpm exec vite --port 5173 --host 127.0.0.1 --strictPort` with `reuseExistingServer: !CI`), `pnpm test:e2e:ui` (UI mode), `pnpm test:e2e:report` (HTML report). `vitest` config lives in `vite.config.ts` `test` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` + `server.watch.ignored` for `skills`/`dist`/`coverage`/`src.orig` (archived St Mary port).
...
pnpm test               # vitest run — 32 files / 175 tests
pnpm test:e2e           # playwright chromium — 48 tests
...
- Tests: `*.test.{ts,tsx}` adjacent to source — **32 files / 175 tests in `src`** (ported 2026-08-31 with Risen Christ fixtures): `src/ci-workflow.test.ts` (4), `src/repo-hygiene.test.ts` (2), `src/docs-contract.test.ts` (8), `src/utils/cn.test.ts` (5), `src/data/nav.test.ts` (7), `src/data/content.test.ts` (10), `src/data/site.test.ts` (8), `src/utils/massDay.test.ts` (5), `src/utils/monogram.test.ts` (7), `src/components/ui/Button.test.tsx` (11), `src/components/SkipLink.test.tsx` (3), `src/components/ui/Accordion.test.tsx` (6), `src/components/SafeImage.test.tsx` (6), `src/components/Header.test.tsx` (17), `src/components/ui/Reveal.test.tsx` (2), `src/components/BackToTop.test.tsx` (7), `src/components/Layout.test.tsx` (2), `src/components/ScrollProgress.test.tsx` (2), `src/components/Timeline.test.tsx` (3), `src/hooks/useScrollProgress.test.ts` (4), `src/hooks/useScrollSpy.test.tsx` (4), `src/pages/Ministries.test.tsx` (3), `src/pages/cta-bands.test.tsx` (6), `src/pages/worship-mass.test.tsx` (5), `src/pages/about-visuals.test.tsx` (4), `src/pages/event-chips.test.tsx` (3), `src/pages/give-featured.test.tsx` (2), `src/pages/card-affordances.test.tsx` (6), `src/pages/NotFound.test.tsx` (2), `src/pages/History.test.tsx` (2), `src/head.test.ts` (13), `src/security-headers.test.ts` (6) + `src/test/setup.ts` (Risen Christ fixtures: priests 3 + ppc 7 + 91 Toa Payoh Central + UEN 4042G + Easter). `vite.config.ts` `test.exclude` keeps `e2e/**` out; `e2e/*.spec.ts` 48 tests are Playwright only. `vite.config.ts` `test.exclude` keeps `e2e/**` out of unit runs; `e2e/*.spec.ts` is Playwright only.

### Design System

- Tokens: see `src/index.css` `@theme`. Additions require design rationale in PR description. Tokens 24 colors + 2 shadows: `shrine-cream/parchment/parchment-dark/stone/ink/charcoal`, `maroon-50..950` (8), `gold-100..600` (5), `pine-500..700` (3), `terracotta-400/500` (2) + `shadow-shrine/shrine-lg`. Only the imagery/content they frame is Toa Payoh 1971 (first air-con nave, Adoration Room, parish hall & media centre) — keep tokens stable.
...
- All five gates are green: `pnpm lint` + `pnpm typecheck` + `pnpm test` (32 files / 175) + `pnpm test:e2e` (40) + `pnpm build` — the full gate `lint && typecheck && test && test:e2e && build` must pass.
...
| 5 | Testing Strategy (wired — 32 files / 175 unit + 48 E2E green, ported 2026-08-31) | Yes | ✅ |
