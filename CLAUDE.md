...
| `pnpm test` / `npm run test` | Vitest `jsdom` — `vitest run` | ⚠️ stale | **No test files in `src/`** — `vitest run` exits 1 (`No test files found`; `src/test/setup.ts` missing). Tests from `src.orig` (St Mary, 25 files / 141 tests) not yet ported for Risen Christ data — see Testing Strategy. Not gated in CI until ported. |
| `pnpm test:watch` | Vitest watch mode (`vitest`) | ⚠️ stale | Watches 0 files until tests are ported |
| `pnpm test:coverage` | Vitest with coverage (`vitest run --coverage`) | ⚠️ stale | No coverage until tests are ported |
| `pnpm test:e2e` / `npm run test:e2e` | Playwright `chromium` — `playwright test` (6 specs in `e2e/`) | ⚠️ stale | Specs still assert St Mary copy (`5 Bukit Batok East Ave 2`, Portiuncula, WOHA) — must be ported for Risen Christ (`91 Toa Payoh Central`, Velankanni, first air-con). Not gated in CI until ported |
...
Current status: **tests not yet ported — gate is `lint && typecheck && build`.** `vitest 3.2.6` (jsdom) + `@testing-library/react 16.2.0` + `jsdom 26.1.0` + `playwright 1.55.1` (chromium) remain installed, but `src/**/*.test.*` is 0 files and `src/test/setup.ts` is missing (so `pnpm test` exits 1). `src.orig/` retains the St Mary suite (25 files / 141 tests + 42 E2E) as reference. Run `pnpm test` (unit), `pnpm test:watch` (watch), `pnpm test:coverage` (coverage), `pnpm test:e2e` (E2E, `webServer` → `pnpm exec vite --port 5173 --host 127.0.0.1 --strictPort` with `reuseExistingServer: !CI`), `pnpm test:e2e:ui` (UI mode), `pnpm test:e2e:report` (HTML report) after porting. `vitest` config lives in `vite.config.ts` `test` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` + `server.watch.ignored` for `skills`/`dist`/`coverage`/`src.orig` (archived St Mary port).
...
src/ (38 source files — tests not yet ported)
  ...
  **/*.test.{ts,tsx}     # (0 files — not yet ported) historical 25 files / 141 tests in src.orig: utils/cn (5), data/nav (7), data/content (10), data/site (7), utils/massDay (5), utils/monogram (7), ui/Button (11), SkipLink (3), ui/Accordion (6), SafeImage (6), Header (16), BackToTop (7), pages/Ministries (3), pages/cta-bands (4), pages/worship-mass (4), pages/about-visuals (3), pages/event-chips (3), components/Timeline (3), pages/NotFound (2), pages/History (2), Layout (2), hooks/useScrollProgress (4), ScrollProgress (2), head (13), security-headers (6)
public/
