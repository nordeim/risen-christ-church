...
- `test` in `vite.config.ts` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` — keeps `e2e/**` out of unit runs. **Note:** `src/test/setup.ts` exists (jest-dom + IntersectionObserver mock + scrollTo/scrollIntoView stubs + matchMedia stub, with Risen Christ fixtures) — `pnpm test` runs the 32 files / 179 unit suite.
...
> Before documenting a command as available, verify it in `package.json` scripts. The five-command gate is fully active (unit 32 files / 179 tests + 48 E2E green since the 2026-08-31 port).
...
Current status: **wired — 32 files / 179 tests + 48 E2E green (2026-08-31).** `vitest 3.2.6` (jsdom) + `@testing-library/react 16.2.0` + `jsdom 26.1.0` + `src/test/setup.ts` (jest-dom + IntersectionObserver + scrollTo/scrollIntoView + matchMedia) + `playwright 1.55.1` (chromium, 7 specs: smoke 11 + navigation 8 + ministries 4 + give-faq 4 + enhancements 7 + enhancements-round5 6 + enhancements-round7 8). Run `pnpm test` (unit), `pnpm test:watch` (watch), `pnpm test:coverage` (coverage), `pnpm test:e2e` (E2E, `webServer` → `pnpm exec vite --port 5173 --host 127.0.0.1 --strictPort` with `reuseExistingServer: !CI`), `pnpm test:e2e:ui` (UI mode), `pnpm test:e2e:report` (HTML report). `vitest` config lives in `vite.config.ts` `test` — `{ globals: true, environment: "jsdom", setupFiles: ["src/test/setup.ts"], include: ["src/**/*.{test,spec}.{ts,tsx}"], exclude: ["e2e/**", "node_modules/**", "playwright-report/**", "test-results/**"] }` + `server.watch.ignored` for `skills`/`dist`/`coverage`/`src.orig` (archived St Mary port).
...
pnpm test               # vitest run — 32 files / 179 tests
...
- All five gates are green: `pnpm lint` + `pnpm typecheck` + `pnpm test` (32 files / 179) + `pnpm test:e2e` (48) + `pnpm build` — the full gate `lint && typecheck && test && test:e2e && build` must pass.
...
| 5 | Testing Strategy (wired — 32 files / 179 unit + 48 E2E green, ported 2026-08-31) | Yes | ✅ |
